  ...
  return (
    <nav className="sticky top-0 h-16 backdrop-blur-md flex items-center justify-between px-4 bg-base-100/50 z-20 border-b-2 border-slate-900/10">
      <ul className="flex gap-5 items-center w-[70%] h-full">
        ...
        <li className="h-full">
          <Link to="/users">
            <div className={`h-full px-2 border-b-4 hover:bg-slate-600/10 hover:border-emerald-600 flex items-center hover:text-emerald-600 font-bold text-sm ${(location.pathname.split("/").length >= 2 && location.pathname.split("/")[1] === "dorms") ? "text-indigo-600 border-indigo-600" : "border-white text-slate-400"}`}>
              Online Users
            </div>
          </Link>
        </li>
